...
def parse_args():
    p = argparse.ArgumentParser(description="Run AE on synthetic data")
    p.add_argument('--variant', choices=['cd', 'ci'], default='cd', help="cd = channel-dependent (AECD), ci = channel-independent (AECI)")
...
def run_one_combo(seed, variant, dataset, latent_dim, window_size):
    ...
    train_loader = DataLoader(Windows(X_train, w=window_size, stride=1),
                              batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
    test_loader = DataLoader(Windows(X_test, labels, w=window_size, stride=window_size),
                             batch_size=BATCH_SIZE, shuffle=False, num_workers=0)

    if variant == 'cd':
        model = AECD(window_size=window_size, latent_dim=latent_dim, n_channels=N_CHANNELS)
    else:
        model = AECI(window_size=window_size, latent_dim=latent_dim, n_channels=N_CHANNELS)
