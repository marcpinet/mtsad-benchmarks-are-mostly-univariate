...
class Basic_CrossAD_CD(nn.Module):
    def __init__(self, configs):
        super(Basic_CrossAD_CD, self).__init__()
        ...
        self.patch_embedding = PatchEmbedding(d_model, patch_len=patch_len, stride=patch_len, padding=(patch_len-1), dropout=0., n_vars=self.n_vars)
        ...
        self.decoder=Decoder(
            ...
            projection=nn.Sequential(nn.Linear(configs.d_model, configs.patch_len * configs.n_vars), nn.Flatten(-2))
        )
        self.context_net=ContextNet(
            router = Router(seq_len=self.ms_t_lens[-1], n_vars=configs.n_vars, n_query=configs.n_query, topk=configs.topk),
    ...
    def _forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        ms_gt = self.ms_utils.concat_sampling_list(ms_x_enc_list[1:] + [x_enc])                 # ms_gt: [bs x ms_t x c]

        # patch_embedding + pos_embedding
        x_enc = x_enc.permute(0, 2, 1)                                                          # x_enc: [bs x c x t]
        _, x_enc = self.patch_embedding(x_enc)                                                   # x_enc: [bs x pn x c*pl]
        for i in range(self.n_scales):
            x_enc_i = ms_x_enc_list[i]
            x_enc_i = x_enc_i.permute(0, 2, 1)                                                  # x_enc_i: [bs x c x t_i]
            x_enc_emb_i, _ = self.patch_embedding(x_enc_i)                                      # x_enc_emb_i: [bs x pn_i x d_model]
            ms_x_enc_list[i] = x_enc_emb_i
        ms_x_enc = self.ms_utils.concat_sampling_list(ms_x_enc_list)                            # ms_x_enc: [bs x ms_pn x d_model]
        ...
        ms_x_enc = ms_x_enc + ms_pos_emb                                                        # ms_x_enc: [bs x ms_pn x d_model]

        # scale-independence encoder
        ms_x_enc, attn_weights = self.encoder(ms_x_enc, self.scale_ind_mask)                    # ms_x_enc: [bs x ms_pn x d_model]

        # period context
        if self.training:
            query_latent_distances, context = self.context_net(router_input, ms_x_enc)           # context: [N*query_len x d_model]
            context = context.unsqueeze(0).expand(bs, -1, -1)                                    # context: [bs x N*query_len x d_model]
            query_latent_distances = query_latent_distances.unsqueeze(-1)                         # query_latent_distances: [bs x 1]
        else:
            query_latent_distances = torch.zeros(bs, 1)
        ...
        ms_x_dec = self.ms_utils.concat_sampling_list(ms_x_dec_list)                                                # ms_x_dec: [bs x up(ms_pn) x d_model]
        ms_x_dec, self_attn_weights, cross_attn_weights = self.decoder(ms_x_dec, context, self.next_scale_mask)     # ms_x_dec: [bs x up(ms_pn)*pl*c]
        ms_x_dec = ms_x_dec.reshape(bs, -1, c)                                                                      # ms_x_dec: [bs x up(ms_pn)*pl x c]
...
class PatchEmbedding(nn.Module):
    def __init__(self, d_model, patch_len, stride, padding, dropout, n_vars=1):
    ...
    def forward(self, x):
        # do patching
        B, C, T = x.shape
        x = self.padding_patch_layer(x)                                         # (B, C, T+pad)
        x = x.unfold(dimension=-1, size=self.patch_len, step=self.stride)       # (B, C, P, Lp)
        P = x.shape[2]
        x = x.permute(0, 2, 3, 1)                                               # (B, P, Lp, C)
        x_patch = x = x.reshape(B, P, self.patch_len * C)                       # (B, P, Lp*C)
        # Input encoding
        x = self.value_embedding(x)                                             # (B, P, d_model)
