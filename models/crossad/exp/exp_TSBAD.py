...
class Exp_Anomaly_Detection():
    def __init__(self, args, id=0):
        ...
        override_latent = getattr(self.args, 'latent_dim', None)
        if override_latent is not None:
            self.model_configs.d_model = int(override_latent)
        override_window = getattr(self.args, 'window_size', None)
        if override_window is not None:
            self.model_configs.seq_len = int(override_window)
    ...
    def _build_model(self):
        if getattr(self.args, 'channel_dependent', False):
            from models.CrossAD.Basic_CrossAD_CD import Basic_CrossAD_CD as Basic_CrossAD
        else:
    ...
    def train(self, data):
        ...
        self._best_state_dict = early_stopping.best_state_dict
    ...
    def test(self, data):
        ...
        if getattr(self, '_best_state_dict', None) is not None:
            print('loading best in-memory model...', end='')
            self.model.load_state_dict(self._best_state_dict, strict=False)
            print('done')
...
class EarlyStopping:
    ...
    def _snapshot(self, val_loss, model):
        if self.verbose:
            print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Caching model state ...')
